...
/// <summary>
/// VolumeSelectionPage の TreeView 表示用ノードの種別を表す列挙型です。
/// </summary>
public enum MaterialVolumeNodeType
{
...
public class MaterialVolumeNode : IDisposable
{
    ...
    public MaterialVolumeNodeType NodeType { get; init; }
    ...
    public MaterialVolumeNode(string name, string fullPath, MaterialVolumeNodeType nodeType = MaterialVolumeNodeType.Folder,
        bool isSelectableByDefault = true, string? selectionDisabledReason = null)
    {
        ...
        this.IsChecked
            .Subscribe(value =>
            {
                if (this.NodeType == MaterialVolumeNodeType.Root)
                {
    ...
    {
        if (this.NodeType == MaterialVolumeNodeType.Archive)
            return false;

        return this.NodeType == MaterialVolumeNodeType.Root
    ...
    private bool isBindingTargetNode()
        => this.NodeType != MaterialVolumeNodeType.Root
        && this.NodeType != MaterialVolumeNodeType.Archive
        && this.CanCheck.Value;
    ...
    private void uncheckCheckedAncestors()
    {
        ...
        {
            if (current.NodeType != MaterialVolumeNodeType.Root
                && current.NodeType != MaterialVolumeNodeType.Archive
    ...
    private void uncheckCheckedDescendants()
    {
        ...
        {
            if (child.NodeType != MaterialVolumeNodeType.Archive && child.IsChecked.Value)
